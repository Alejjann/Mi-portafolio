    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
      {articles.map((article, i) => (
        <motion.article
          ...
          className={`p-6 rounded-xl shadow transition-colors duration-300 ${darkMode ? "bg-gray-800 text-gray-100" : "bg-gray-100 text-gray-800"}`}
          role="article"
          aria-labelledby={`blog-article-${i}`}
        >
          <h3 id={`blog-article-${i}`} className="font-bold text-2xl mb-2">
            {article.title}
          </h3>
          <p className="mb-4 text-base">{article.summary}</p>
          <a
            href={article.link}
